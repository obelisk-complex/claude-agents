...
- **When a change alters established behaviour, sweep for sibling staleness.** A fix landing correctly in one place is not the whole fix if another artefact still asserts, restates, or depends on the behaviour just changed: a comment or rationale string elsewhere carrying the old reasoning, a doc paragraph whose "the one exception" is now three, a sibling test whose inline comment only holds under the old value, a second copy of the same claim in another file. Grep for the old value, name, or claim beyond the diff, not just within it. A correct fix next to a now-false comment is a finding, not a clean pass - the false comment reads as authoritative to the next person who trusts it.
